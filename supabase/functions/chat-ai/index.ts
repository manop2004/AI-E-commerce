...
IN-CHAT CART & CHECKOUT (สำคัญมาก — ใช้แทนการสร้างออเดอร์ตรงๆ):
- เมื่อลูกค้าพูดว่า "เอา/สั่ง/ใส่ตะกร้า/add to cart/I'll take" → ตอบสั้นๆ แล้วต่อท้ายด้วย marker (เราจะลบก่อนแสดงผล):
  <<CART_ADD:ชื่อสินค้าตรงตาม CATALOG|จำนวน>>
- ลูกค้าจะเห็นตะกร้าเด้งขึ้นในแชท แล้วกด "ยืนยันสั่งซื้อ" เพื่อกรอกที่อยู่เอง — บอทไม่ต้องถามที่อยู่
- ถ้าลูกค้าเปลี่ยนใจอยากเอาออก ใช้ <<CART_REMOVE:ชื่อสินค้า>>
- หลังใส่ตะกร้า ให้ชวนต่อยอด เช่น "เพิ่มอะไรอีกไหมคะ? หรือกดยืนยันสั่งซื้อด้านล่างเลย"

LEGACY ORDER MARKER (ใช้เฉพาะกรณีพิเศษ — ปกติให้ใช้ CART_ADD แทน):
${orderingEnabled ? "- <<ORDER:name|qty>> จะสร้างออเดอร์ทันทีโดยไม่มีที่อยู่ — เลี่ยงใช้ ให้ใช้ CART_ADD แทนเสมอ" : "- ห้ามใช้ <<ORDER:...>> เด็ดขาด"}

CUSTOMER ANALYSIS (วิเคราะห์ลูกค้าอัตโนมัติ — ต่อท้ายทุกครั้งที่มีข้อมูลใหม่):
- ต่อท้ายข้อความด้วย marker JSON นี้ (เราจะลบก่อนแสดงผล ลูกค้าไม่เห็น):
  <<META:{"intent":"browsing|comparing|ready_to_buy|support|complaint","categories":["..."],"budget":"low|mid|high|unknown","score":1-100,"lang":"th|en|zh|...","summary":"สรุปสั้นๆ ภาษาไทย"}>>
- intent: ความตั้งใจ, categories: หมวดสินค้าที่สนใจ, budget: ช่วงงบ, score: ความน่าจะปิดดีล 1-100, lang: ภาษาที่ลูกค้าใช้, summary: สรุปลูกค้า 1 ประโยค
- ถ้าไม่มีข้อมูลใหม่ ไม่ต้องใส่ META`;
    ...
    let reply = rawReply;
    const cartActions: { action: "add" | "remove"; name: string; qty: number }[] = [];

    // Parse CART_ADD / CART_REMOVE markers (client maintains cart)
    const cartAddRegex = /<<CART_ADD:([^|>]+)\|(\d+)>>/g;
    for (const m of rawReply.matchAll(cartAddRegex)) {
      cartActions.push({ action: "add", name: m[1].trim(), qty: parseInt(m[2], 10) || 1 });
    }
    const cartRemoveRegex = /<<CART_REMOVE:([^>]+)>>/g;
    for (const m of rawReply.matchAll(cartRemoveRegex)) {
      cartActions.push({ action: "remove", name: m[1].trim(), qty: 0 });
    }
    reply = reply.replace(cartAddRegex, "").replace(cartRemoveRegex, "").trim();

    // Parse META analysis marker and persist on conversation
    const metaMatch = reply.match(/<<META:(\{[\s\S]*?\})>>/);
    if (metaMatch && conversationId) {
      try {
        const meta = JSON.parse(metaMatch[1]);
        const update: any = {};
        if (meta.intent) update.customer_intent = String(meta.intent).slice(0, 64);
        if (Array.isArray(meta.categories)) update.interested_categories = meta.categories.slice(0, 8).map((c: any) => String(c));
        if (meta.budget) update.budget_range = String(meta.budget).slice(0, 32);
        if (typeof meta.score === "number") {
          update.lead_score = Math.max(0, Math.min(100, Math.floor(meta.score)));
          if (update.lead_score >= 70) update.lead_tag = "hot";
          else if (update.lead_score >= 40) update.lead_tag = "warm";
          else update.lead_tag = "cold";
        }
        if (meta.lang) update.preferred_language = String(meta.lang).slice(0, 8);
        if (meta.summary) update.summary = String(meta.summary).slice(0, 500);
        if (Object.keys(update).length) {
          await admin.from("conversations").update(update).eq("id", conversationId);
        }
      } catch (_) {}
    }
    reply = reply.replace(/<<META:\{[\s\S]*?\}>>/g, "").trim();
    ...
    return json({ reply, conversationId, cartActions });
  } catch (e) {
